...
| paystyle   | 字符串     | 是    | wx   | 支付方式::</br>ali (支付宝) ，</br>wx (微信)，</br> bil(快钱)，</br> unionPay(银联)，</br> health_insurance（医保），</br>cash（现金）,</br>prePaid(储值卡),</br>coupon(购物券)</br>wx_wx_huifu(微信),</br>ali_ali_huifu(支付宝),</br>micro_wx_huifu(微信),</br>micro_ali_huifu(支付宝) |
